...
MyGame = ig.Game.extend({
	...
	imgTrap: new ig.Image( 'media/purplebox.png' ),
	...
	init: function() {
		...
		this.createFloor(1, 1);
		
		this.initKeys();
	},

	createFloor: function  (amountOfPlayers, floorNumber) {
		this.mazeSolvable = false;
		while(!this.mazeSolvable){
			this.generateMaze(amountOfPlayers, floorNumber);
		}
	},

	generateMaze: function (amountOfPlayers) {
		this.level = [];
		for (var i = 0; i < 64; i++) {
			if(Math.random() > 0.3){
				this.level.push(1);
	...
	generateNextLevel: function (nextFloorNumber) {
	},
	...
	initKeys: function(){
		...
		ig.input.bind( ig.KEY.SPACE, this.moveID);
		//ig.input.bind( gamepad.FACE_2, this.moveID);
		//ig.input.bind( gamepad.FACE_4, this.moveID);

		this.upID = 'up' + 1;
		ig.input.bind( ig.KEY.W, this.upID);
		this.downID = 'down' + 1;
		ig.input.bind( ig.KEY.S, this.downID);
		this.leftID = 'left' + 1;
		ig.input.bind( ig.KEY.A, this.leftID);
		this.rightID = 'right' + 1;
		ig.input.bind( ig.KEY.D, this.rightID);
		this.inventoryID = 'inventory' + 1;
		ig.input.bind( ig.KEY.I, this.inventoryID);
		
	},

	placePlayer: function(number){

		var player = ig.game.spawnEntity(EntityPlayer, 5, 5, {playerNumber: number});
	...
	placeTrap: function(){
		...
		while(!trapPlaced){
			var placement = Math.round(Math.random()*63);
			if(that.level[placement] === 1){
				that.level[placement] = 7;
				trapPlaced = true;
				that.trapPlacement = placement;
			}
		}
	},
	...
	drawMaze: function () {
		...
		this.level.forEach(function(entry){
			if(entry === 0){
				...
				that.imgTrap.draw( (count%8)*8+2, Math.floor(count/8)*8+2 );
			} else if(entry === 8){
...
// up by a factor of 2
ig.main( '#canvas', MyGame, 60, 128, 128, 1 );
