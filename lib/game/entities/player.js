...
EntityPlayer = ig.Entity.extend({
		...
		setPlayerImage: function () {
			if(this.playerNumber === 1){
				this.playerImage = new ig.Image( 'media/redbox.png' );
			} else if(this.playerNumber === 2){
				this.playerImage = new ig.Image( 'media/greenbox.png' );
			} else if(this.playerNumber === 3){
				this.playerImage = new ig.Image( 'media/bluebox.png' );
			} else if(this.playerNumber === 4){
				this.playerImage = new ig.Image( 'media/purplebox.png' );
			} else if(this.playerNumber === 5){
				this.playerImage = new ig.Image( 'media/whitebox.png' );
			}
		},
		...
		update: function() {
			...
			if(this.movingTimer.delta()>0 && !this.won){
				if(ig.input.pressed(this.upID)){
					...
					ig.game.level[this.playerPlacement] = 1;
					this.dungeonFloor++;
					if(ig.game.allPlayersWon()){
						ig.game.generateNextLevel(this.dungeonFloor);
					}
		...
		drawMe: function () {
			this.playerImage.draw( this.pos.x + this.playerNumber * 20, this.pos.y + 72 );
		},
			...
			if(!this.won){
				if(this.checkInventory){
					...
					if(this.investigatedTile === 1){
						this.playSound = false;
					} else if(this.investigatedTile === 0 || this.investigatedTile === 2 ){
